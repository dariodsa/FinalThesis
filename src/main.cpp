...
int main(int argc, char* argv[]) {
    ...
    /*
    //5**
    Select* sa = process_query(replicas[0], "select l_tax from lineitem union select ps_suppkey from partsupp union select p_partkey from part union select o_orderkey from orders;");
    printf("5**: Cost: %lf\n\n\n\n", sa->getFinalCost(replicas[0]));

    //5*
    Select* sa2 = process_query(replicas[0], "select l_tax from lineitem union select ps_suppkey from partsupp;");
    ...
    queries.push_back(make_pair("select l_tax from lineitem union select ps_suppkey from partsupp union select p_partkey from part union select o_orderkey from orders;", 6));
